...
import '../../../../../app/server/fetch_api.dart';
...
class ChatRoomController extends GetxController {
  ...
  void onInit() async {
    ...
    chatUserToken = user.data()?["metadata"]?[Keys.token];

    Debug.log("ChatUserToken:::: $chatUserToken");
  ...
  void handleSendPressed(types.PartialText message) async {
    ...
    if (!GetUtils.isNullOrBlank(chatUserToken)!) {
      var payload = {
      ...
      Get.find<PushNotificationsManager>().sendNotifications(
        chatUserToken!,
  ...
  Future<void> onImageSelection(
    ...
    final imagePath = await ApiFetch.imageUploading(file, (_) {});
